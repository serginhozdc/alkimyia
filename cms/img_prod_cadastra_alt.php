...
if ($_REQUEST['apagar']) {
	$sql = "SELECT * FROM img_produtos WHERE id={$_REQUEST["cod"]}";
	$result = mysql_query($sql);
	$linha = mysql_fetch_assoc($result);
	
	$sql = "DELETE FROM  img_produtos WHERE id={$_REQUEST['cod']}";
	if (unlink("../upload/"+$linha["img"]) && mysql_query($sql)) {
		alert("Imagem excluída com sucesso!");
		redireciona("img_prod_cadastra_alt.php?id=$id_produtos");
	}
	...
	if (!$ok) {
		...
		alert("Algum campo foi preenchido incorretamente ou está em branco, tente novamente!");
			...
			$dir = "../upload";
				...
				$sql = "INSERT INTO img_produtos (img,id_produtos) VALUES ('$arquivo','$id_produtos')";	
				...
				if (mysql_query($sql)) {
					alert("Imagem cadastrada com sucesso! $id_produtos");
					redireciona("img_prod_cadastra_alt.php?id=$id_produtos");
				}
...
<br><br><br></div>
	...
	<div id="global">
		...
		<div id="principal">
			...
			<form action="img_prod_cadastra_alt.php?cadastra=1&id=<?=$id_produtos?>" method="post" name="cadastro" enctype="multipart/form-data">
			...
			<div id="caixa_table">
				...
				<?
					$sql = "SELECT * FROM img_produtos WHERE id_produtos=$id_produtos";
					//echo $sql;
					$result = mysql_query($sql);
				?>
				<?
				...
				?>					
					<tr height="30" class="cel_fonte" onMouseOver="this.className='cel_fonte_hover'" onMouseOut="this.className='cel_fonte'">
						...
						<td align="center" width="1%" nowrap="nowrap">
							...
							-->
							<a href="img_prod_cadastra_alt.php?apagar=1&cod=<?=$linha["id"]?>&id=<?=$id_produtos;?>"><img src="images/icon_apagar.gif" onclick="javascript: return confirm('Deseja realmente excluir a Imagem ?')" alt="Apagar" border="0"></a></td>
					</tr>
				<?
				}
				?>
